...
Group is the main thing we have for assigning a category to a log message.
But it makes more sense to think of this as a set of tags, than as a single category.

For example, the kinds of tags a system doing data processing might have includes
`missing-data`, `model-fitting-issue`, `alert-pager` etc.
and we could want to do `alert-pager` and `missing-data`
...
`group` is avaiable, and by default is set to the file
but it might be being used for something else
